...
void setup(void) {
 ...
 while (SD.exists(dataString)) {
  ...
  outputFile.print(F(","));
  outputFile.println(F("gTm,aX,aY,aZ,gX,gY,gZ,rotnX,rotnY,rotnZ,offVert,vel,alt,events,cont,fire,pin,highGx,Alt,press,temp,magX,magY,magZ,lat,lon,speed,gps_alt,gps_angle,satellites,packet"));
  outputFile.sync();
...
void loop(void){
  ...
  if (liftoff) {
    ...
    if(!apogee || testMode){
      ...
      else{frac = accelVel/300; radioVel = (int16_t)((frac)*accelVel + (1.0 - frac)*baroVel);}
    ...
    //Cycle timestamp
    writeLongData(timeCurrent, 2, num0);
    writeIntData(timeGyro-timeCurrent);
    //LSM9DS1 Accel Data
    writeIntData(accelX);
    writeIntData(accelY);
    writeIntData(accelZ);
    ...
    if(gpsWrite){
      ...
      writeLongData(gpsLatitude,2,6);
      dataString[strPosn]=gpsLon;strPosn++;
      writeLongData(gpsLongitude,2,6);
    ...
    if (timeOut || touchdown) {
      ...
      outputFile.println(F("Max Baro Alt,Max GPS Alt,Max Speed,Max Gs,baseAlt,initial Y ang,initial Z ang,accelX0,accelY0,accelZ0,highGx0,magX0,magY0,magZ0,gyroBiasX,gyroBiasY,gyroBiasZ,accelBiasX,accelBiasY,accelBiasZ,highGxBias"));
      writeLongData(long(maxAltitude*3.2808), 2, 0);
      writeLongData(long(maxGPSalt*3.2808), 2, 0);
      writeLongData(long(maxVelocity*3.2808), 2, 0);
      ...
      dataString[strPosn]=liftoffLat; strPosn++; writeLongData(liftoffLatitude,2,4);
      dataString[strPosn]=liftoffLon; strPosn++; writeLongData(liftoffLongitude,2,4);
      //end of sample - carriage return, newline, and null value
      dataString[strPosn] = '\r';strPosn++;
      dataString[strPosn] = '\n';strPosn++;
      dataString[strPosn] = '\0';
      ...
      dataString[strPosn]=touchdownLat; strPosn++; writeLongData(touchdownLatitude,2,4);
      dataString[strPosn]=touchdownLon; strPosn++; writeLongData(touchdownLongitude,2,4);
      //end of sample - carriage return, newline, and null value
  ...
  if (GPS.location.isUpdated() || GPS.altitude.isUpdated()) {
        timeLastGPS = micros();
        gpsFix = 1;
        if(!configGPSflight){configGPS();configGPSflight = true; configGPSdefaults = false;}
        gpsWrite = true;
        gpsTransmit = true;
        convertLocation();
...
void writeLongData(float dataValue, byte dataLen, byte decimals) {
...
void writeFloatData(float dataValue, byte decimals){
  ...
  //fractional portion
  partial = dataValue - (int)(dataValue);//-.009
  if(partial < 0.0){partial*= -1;}//0.09
  fracInt = (int)(partial*pow(10,decimals));//9 
  if(fracInt == 0){dataString[strPosn] = '0'; strPosn++; dataString[strPosn] = cs; strPosn++;}
    ...
    itoa(fracInt, dataString + strPosn, base);
    while(dataString[strPosn]!= '\0'){strPosn++;}
    dataString[strPosn]=','; strPosn++;}}
