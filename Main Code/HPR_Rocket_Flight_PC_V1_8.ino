...
//V1_8 increases telemetry rate w/ ground station code overhaul
...
//950Hz of flight events
//950Hz of integrated speed, altitude, rotation, continuity
...
void loop(void){
  ...
  if (liftoff) {
    ...
    if(!apogee || testMode){
      ...
      accelVel += (accelNow - g) * float(dt) * 0.11768 * mlnth;
    ...
    if (checkFalseTrigger) {
      ...
      if (accelX < gTrigger && accelVel < 5.0F) {
    ...
    if (!apogee && boosterBurnout && !boosterBurnoutCheck && !pyroFire && (accelVel < 0 || (baroApogee < 0 && accelVel < 70))) {
      apogee = true;
      timeLastEvent = timeCurrent;}
    ...
    if(timeCurrent - timeLastSample > sampDelay){//33 byte first sample, 19 bytes after
      ...
      //Acceleration
      dataPacket[pktPosn] = lowByte(accelNow);pktPosn++;//18
      dataPacket[pktPosn] = highByte(accelNow);pktPosn++;//19
      //GPS Data
      if(TXgps){
