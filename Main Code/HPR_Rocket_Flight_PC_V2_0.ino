...
boolean radioTXmode = false;//false turns off radio transmissions
//-----------------------------------------
...
void setup(void) {
  ...
  rf95.setTxPower(13, false);//23 max setting; 20mW=13dBm, 30mW=15dBm, 50mW=17dBm, 100mW=20dBm
 ...
 //Read the battery votage from the main parachute continuity pin
  ...
  //check for silent mode, only available through programming
  if(silentMode){beepPin = 32;}
  ...
  if (testMode){
    ...
    Serial.println("Test Mode Confirmed");
    beep_counter = 0;
  ...
  if(calibrationMode){
    ...
    Serial.println("Calibration Mode Confirmed. Please orient altimeter");
    ...
    Serial.println("Calibrating - Hold altimeter still");
    ...
    digitalWrite(beepPin, LOW);
    Serial.println("Calibration complete");}//end calibration mode
  ...
  if(testMode){Serial.println("Reading SD Card");}
 ...
 // Rename the data file to FLIGHT01.txt
  ...
  if(testMode){Serial.print("Creating new SD card file: FLIGHT");}
 ...
 while (SD.exists(dataString)) {
  ...
  if(testMode){Serial.print("Reporting continuity: ");Serial.print(beepCode);}
  ...
  if(testMode){Serial.println("Hold Rocket Vertical");}
  ...
  if(testMode){Serial.println("Sampling Gyro");}
  ...
  if(testMode){Serial.println("Sampling complete");}
  ...
  if(testMode){Serial.println("Reporting Main Deploy Settings");}
  ...
  if(testMode){Serial.print("Reporting last flight");}
  ...
  if(testMode){Serial.print("Reporting Battery Voltage: ");Serial.print(voltage);}
  ...
  if(testMode){Serial.println("Setup Complete.  Awaiting simulated launch");}
}//end setup
...
void loop(void){
  ...
  if (liftoff) {
    ...
    if (!apogee && boosterBurnout && !boosterBurnoutCheck && !pyroFire && (accelVel < 0 || (baroApogee < 0 && accelVel < 70 && Alt < 30000))) {
      apogee = true;
      timeLastEvent = timeCurrent;}
    ...
    //build the packet of 4 samples
    if(timeCurrent - timeLastSample > sampDelay){//33 byte first sample, 19 bytes after
      timeLastSample = timeCurrent;
      sampNum++;
      //event
      dataPacket[pktPosn]=radioEvent; pktPosn++;//1
      //time
      timeUnion.radioTime = timeCurrent;
      for(byte i = 0; i < 4; i++){dataPacket[pktPosn]=timeUnion.timeBytes[i];pktPosn++;}//5
      //integrated velocity
      ...
      dataPacket[pktPosn] = lowByte(radioAccelAlt);pktPosn++;//8
      dataPacket[pktPosn] = highByte(radioAccelAlt);pktPosn++;//9
      //barometric altitude
      radioBaroAlt = (int16_t)(Alt);
      dataPacket[pktPosn] = lowByte(radioBaroAlt);pktPosn++;//10
      dataPacket[pktPosn] = highByte(radioBaroAlt);pktPosn++;//11
      //Rotation data
      dataPacket[pktPosn] = lowByte(rotnX);pktPosn++;//12
      dataPacket[pktPosn] = highByte(rotnX);pktPosn++;//13
      dataPacket[pktPosn] = lowByte(rotnY);pktPosn++;//14
      dataPacket[pktPosn] = highByte(rotnY);pktPosn++;//15
      dataPacket[pktPosn] = lowByte(rotnZ);pktPosn++;//16
      dataPacket[pktPosn] = highByte(rotnZ);pktPosn++;//17
      //Acceleration
      radioAccelNow = (int)(accelNow * 33.41406087); //33.41406087 = 32768 / 9.80665 / 100
      dataPacket[pktPosn] = lowByte(radioAccelNow);pktPosn++;//18
      dataPacket[pktPosn] = highByte(radioAccelNow);pktPosn++;//19
      //GPS Data
      if(TXgps){
        radioGPSalt = (int16_t)(GPS.altitude.meters());
        dataPacket[pktPosn] = lowByte(radioGPSalt);pktPosn++;//20
        dataPacket[pktPosn] = highByte(radioGPSalt);pktPosn++;//21
        dataPacket[pktPosn]=gpsLat;pktPosn++;//22
        GPSunion.GPScoord = gpsLatitude;
        for(byte i = 0; i < 4; i++){dataPacket[pktPosn]=GPSunion.GPSbyte[i];pktPosn++;}//26
        dataPacket[pktPosn]=gpsLon;pktPosn++;//27
        GPSunion.GPScoord = gpsLongitude;
        for(byte i = 0; i < 4; i++){dataPacket[pktPosn]=GPSunion.GPSbyte[i];pktPosn++;}//31
        TXgps = false;
        packetnum++;
        dataPacket[pktPosn] = lowByte(packetnum); pktPosn++;//32
        dataPacket[pktPosn] = highByte(packetnum); pktPosn++;}}//33
    if(sampNum >= 4){
