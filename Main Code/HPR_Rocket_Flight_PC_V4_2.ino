...
//Code Line Count: 2939 main + 328 GPSconfig + 0 KalmanFilter + 180 Rotation + 1758 SensorDrivers + 536 SpeedTrig + 119 Telemetry = 5860
//-----------Change Log------------
//V4_0 is compatible across multiple hardware configurations and can be mounted in any orientation
...
//V4_2 adds 915MHz capability
//--------FEATURES----------
//1500Hz 3-axis digital 24G and 100G accelerometer data logging
//1500Hz 3-axis digital 2000dps gyroscope data logging
//1500Hz of flight events & continuity
//1500Hz of integrated speed & altitude
//100Hz to 500Hz of user selectable quaternion rotation
...
//10Hz-25Hz of GPS data logging (chip dependent data rates & constellations)
...
//Bench-test mode activated w/ tactile button; sends status messages over USB
...
//Inflight Powerloss Restart Detection
...
//  adc->adc0->setSamplingSpeed(ADC_SAMPLING_SPEED::VERY_HIGH_SPEED);
  ...
  if (settings.testMode){
    ...
    while(beep_counter < 8){
      ...
      if(digitalRead(pins.testRead) == LOW && !settings.testMode){
        delay(50);//necessary because sometimes when the button is released it pings back
 ...
 while (SD.exists(dataString)) {
  ...
  outputFile.println(F(",accelX,accelY,accelZ,gyroX,gyroY,gyroZ,highGx,highGy,highGz,smoothHighGz,roll,yaw,pitch,offVert,intVel,intAlt,fltEvents,pyroCont,pyroFire,pyroPin,baroAlt,altMoveAvg,baroVel,baroPress,baroTemp,battVolt,magX,magY,magZ,magRoll,magYaw,magPitch,magOffVert,gnssLat,gnssLon,gnssSpeed,gnssAlt,gnssAngle,gnssSatellites,radioPacketNum"));
  outputFile.sync();
...
void loop(void){
  ...
  if (events.liftoff) {
    ...
    if(newBMP){
      ...
      (events.mainDeploy && abs(baroVel) < 1.0F )? baroTouchdown++ : baroTouchdown = 0;}
