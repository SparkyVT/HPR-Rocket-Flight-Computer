...
//Code Line Count: 8916 lines of code = 2336 MainFile + 352 Bus_Mgmt + 2198 SensorDrivers + 885 Calibration + 625 SpeedTrig + 507 Inflight_Recover + 636 SD + 397 Rotation + 375 Telemetry + 320 Event_Logic + 283 GPSconfig      
//--------FEATURES----------
...
void setup(void) {
  ...
  pinMode(pins.radioIRQ, INPUT_PULLUP);
...
void loop(void){
  ...
  if (events.liftoff) {
    ...
    if(accel.newSamp || gyro.newSamp || highG.newSamp || mag.newSamp || baro.newSamp || baro.newTemp || gpsWrite || writeVolt || radioTX){
      writeSDflightData();}
