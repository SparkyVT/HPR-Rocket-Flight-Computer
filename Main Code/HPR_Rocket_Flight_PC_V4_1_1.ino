...
//Code Line Count: 2894 main + 281 GPSconfig + 0 KalmanFilter + 139 Rotation + 1736 SensorDrivers + 480 SpeedTrig + 153 Telemetry = 5683
//-----------Change Log------------
...
//Kalman Filter
//modify packets to transmit a minimum of data post-apogee
...
void setup(void) {
  ...
  if(SD.exists("EEPROMsettings.txt")){
    ...
    //58   : radio interrupt pin
    //59   : radio reset pin
    //60   : radio enable pin (optional w/ Adafruit breakout only)
    //61   : Sensor accel / mag 
    //62   : Sensor gyro
    //63   : Sensor baro
    //64   : Sensor highG
    ...
    //66   : Report Style
    //67   : Units Metric or standard
    //68-71: Least Squares Estimate of ADXL377 HighG Gain
    //72   : servo 1 control pin
    //73   : servo 2 control pin
    //74   : servo 3 control pin
    //75   : servo 4 control pin
    //76   : servo 5 control pin
    //77   : servo 6 control pin
    //78   : servo 7 control pin
    //79   : servo 8 control pin
    ...
    parseNextVariable(false); EEPROM.update(66, (char)dataString[0]);//Report Style
    parseNextVariable(false); EEPROM.update(67, (char)dataString[0]);//Units Metric or Standard
    ...
    for(byte i = 0; i < 4; i++){EEPROM.update(68+i, calFloat.calByte[i]);}////ADXL377 Gain
    for(byte j = 72; j < 80; j++){//Servo control pins
      kk = (byte)parseNextVariable(true);Serial.print(kk);Serial.print(F(", "));
      EEPROM.update(j, kk);}
    settingsFile.close();
  ...
  settings.units = (char)EEPROM.read(j);j=j+5;
  ...
  //Serial.print("Pyro4 analog read: ");Serial.println(analogRead(pins.pyro4Cont));
  ...
  //digitalWrite(pins.pyro4Fire, HIGH);
  //delay(100);
  //digitalWrite(pins.pyro4Fire, LOW);
...
//  adc->adc0->setSamplingSpeed(ADC_SAMPLING_SPEED::VERY_HIGH_SPEED);
  ...
  if (settings.rotnCalcRate < 2000UL){settings.rotnCalcRate = 2000UL;}//max 500 quaternion samples per second
  if (sensors.radio == 2){settings.TXfreq = 902.300 + 0.2 * chnl[freqNum]; settings.TXpwr = 13;}
  ...
  if(settings.radioTXenable){
    ...
    if(settings.testMode && sensors.radio == 2){settings.TXfreq = 915.000F;}
 ...
 if(pyro4.contStatus){
  ...
  if(!pyro1.contStatus && !pyro2.contStatus && !pyro3.contStatus && !pyro4.contStatus){cont.reportCode = 10;}
  ...
  if (!cont.error && (settings.fltProfile == 'S' || settings.fltProfile == 'B')){
    ...
    else {cont.beepCode = 0; cont.reportCode = 10;}
    postFlightCode = 1;}
...
void loop(void){
  ...
  if (!events.liftoff && accel.z > settings.gTrigger && !events.touchdown && !events.timeOut) {
    ...
    if(settings.radioTXenable){radioSendPacket(); lastTX = fltTime.timeCurrent;}
  ...
  if (events.liftoff) {
    ...
    if(newBMP){
      ...
      if(fltTime.timeCurrent < 500000L){baroVel = 0;}
    ...
    if (pyroFire) {
      ...
      //Turn off the pyros
      if(pyro1.fireStatus && fltTime.timeCurrent > pyro1.fireStop){digitalWrite(pyro1.firePin, LOW);pyro1.fireStatus = false;}
      if(pyro2.fireStatus && fltTime.timeCurrent > pyro2.fireStop){digitalWrite(pyro2.firePin, LOW);pyro2.fireStatus = false;}
      if(pyro3.fireStatus && fltTime.timeCurrent > pyro3.fireStop){digitalWrite(pyro3.firePin, LOW);pyro3.fireStatus = false;}
      if(pyro4.fireStatus && fltTime.timeCurrent > pyro4.fireStop){digitalWrite(pyro4.firePin, LOW);pyro4.fireStatus = false;}
    ...
    if (!events.touchdown && events.mainDeploy && !pyroFire && !settings.testMode && baroTouchdown > touchdownTrigger && Alt < 46) {
      ...
      radioInterval = RIpostFlight;
      if(settings.radioTXenable){radioSendPacket();lastTX = micros();}
      //radioTimer.begin(radioSendPacket, 30000000);
      //if(settings.testMode){radioTimer.begin(radioSendPacket, 1000000);}
      radio.event = 27;
      touchdownHour = GPS.time.hour();
      touchdownMin = GPS.time.minute();
      touchdownSec = GPS.time.second();
    ...
    if(settings.radioTXenable && fltTime.timeCurrent - lastTX >= radioInterval){radioSendPacket(); lastTX += radioInterval;}
    ...
    /*if(strPosn >= 512){ 
      writeStart = micros();
      outputFile.write(dataString, 512);
      writeTime = micros() - writeStart;
      if(writeTime > maxWriteTime){maxWriteTime = writeTime;}
      if(writeTime > writeThreshold){writeThreshCount++;}
      memcpy(dataString, dataString+512, strPosn - 512);
      strPosn -= 512;}*/
    ...
    if (events.timeOut || events.touchdown) {
      ...
      radio.maxG = (int)((maxG / 9.80665) * 327.68);
  ...
  //radio handling preFlight and postFlight
  if(settings.radioTXenable && (events.preLiftoff || events.postFlight) && micros() - lastTX >= radioInterval){radioSendPacket(); lastTX += radioInterval;}
  ...
  while(HWSERIAL.available() > 0){
    ...
    GPS.encode(c);
    if(settings.testMode && !events.liftoff){Serial.print(c);}
...
void firePyros(char event){
   pyroFire = true;
   if(pyro1.func == event){firePin = pyro1.firePin; digitalWrite(pyro1.firePin, HIGH); pyro1.fireStatus = true; pyro1.fireStop = fltTime.timeCurrent + settings.fireTime;}
   if(pyro2.func == event){firePin = pyro2.firePin; digitalWrite(pyro2.firePin, HIGH); pyro2.fireStatus = true; pyro2.fireStop = fltTime.timeCurrent + settings.fireTime;}
   if(pyro3.func == event){firePin = pyro3.firePin; digitalWrite(pyro3.firePin, HIGH); pyro3.fireStatus = true; pyro3.fireStop = fltTime.timeCurrent + settings.fireTime;}
   if(pyro4.func == event){firePin = pyro4.firePin; digitalWrite(pyro4.firePin, HIGH); pyro4.fireStatus = true; pyro4.fireStop = fltTime.timeCurrent + settings.fireTime;}}
