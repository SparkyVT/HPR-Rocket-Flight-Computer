...
void setup(void) {
 ...
 while (SD.exists(dataString)) {
  ...
  outputFile.print(F(","));
  outputFile.println(F("accelX,accelY,accelZ,gyroX,gyroY,gyroZ,rotnZ,rotnY,rotnX,offVert,intVel,intAlt,fltEvents,pyroCont,pyroFire,pyroPin,highGx,highGy,highGz,highGavg10,Alt,press,temp,magX,magY,magZ,lat,lon,speed,gps_alt,gps_angle,satellites,packet"));
  outputFile.sync();
...
void loop(void){
  ...
  if (liftoff) {
    ...
    if (timeOut || touchdown) {
      ...
      outputFile.println(F("Max Baro Alt,Max GPS Alt,Max Speed,Max Gs,baseAlt,initial Y ang,initial X ang,accelX0,accelY0,accelZ0,highGx0,highGy0,highGz0,magX0,magY0,magZ0,gyroBiasX,gyroBiasY,gyroBiasZ,accelBiasX,accelBiasY,accelBiasZ,highGxBias,highGyBias,highGzBias"));
  ...
  if(radioTXmode && onGround && micros() - lastTX > radioDelay){
    ...
    if(touchdown || timeOut){//22 byte packet
      ...
      GPSunion.GPScoord = gpsLongitude;
      for(byte i = 0; i < 4; i++){dataPacket[pktPosn]=GPSunion.GPSbyte[i];pktPosn++;}
      Serial.print("Radio Posn: ");Serial.println(pktPosn);
      rf95.setModeIdle();
      rf95.send((uint8_t *)dataPacket, pktPosn);
      lastTX = micros();}
...
float parseNextVariable(boolean flag){
  ...
  n=0;
  c='\0';
  while (c != '='){c = settingsFile.read();}//move to the equal sign
  c = settingsFile.read();
