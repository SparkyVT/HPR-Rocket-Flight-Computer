...
void setup(void) {
 ...
 //Read the battery votage from the main parachute continuity pin
  ...
  delay(50);
  if(digitalRead(buttonRead) == LOW){testMode = true; Serial.println("Test Mode Confirmed");}

 if(testMode){Serial.println("Reading User Settings from SD Card");}
  ...
  if(silentMode){beepPin = 4;}
  if(testMode && silentMode){Serial.println("Silent Mode Confirmed");}
  ...
  if(testMode && !radioTXmode){Serial.println("Telemetry OFF!");}
  
  //signal if in test-mode
  if (testMode){

    Serial.println("Signaling Test Mode");
    beep_counter = 0;
 ...
 while (SD.exists(dataString)) {
  ...
  if(testMode){Serial.print("Reporting continuity: ");Serial.println(beepCode);}
  ...
  if(testMode){Serial.print("Reporting Main Deploy Settings: ");Serial.println((int)(mainDeployAlt*3.2808-1));}
  ...
  if(testMode){Serial.println(" "); Serial.print("Reporting Battery Voltage: ");Serial.println(voltage);}
...
void loop(void){
  ...
  if (liftoff) {
    ...
    if(radioTXmode && timeCurrent - timeLastSample > sampDelay){//33 byte first sample, 19 bytes after
      ...
      //Rotation data
      dataPacket[pktPosn] = lowByte(rollZ);pktPosn++;//12
      dataPacket[pktPosn] = highByte(rollZ);pktPosn++;//13
      dataPacket[pktPosn] = lowByte(yawY);pktPosn++;//14
      dataPacket[pktPosn] = highByte(yawY);pktPosn++;//15
      dataPacket[pktPosn] = lowByte(pitchX);pktPosn++;//16
      dataPacket[pktPosn] = highByte(pitchX);pktPosn++;//17
      //Acceleration
