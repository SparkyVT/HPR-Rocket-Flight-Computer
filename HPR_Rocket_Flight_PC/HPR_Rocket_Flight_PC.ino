...
//--------FEATURES----------
//850Hz 3-axis digital 24G and 200G accelerometer data logging
//850Hz 3-axis digital 2000dps gyroscope data logging
//850Hz of flight events
//850Hz of integrated speed, altitude, rotation
...
void setup(void) {
 ...
 //Read the battery votage from the main parachute continuity pin
  ...
  if(calibrationMode){
    ...
    for (byte i = 1; i < 31; i++){
      ...
      accelBiasY += accelY;
      accelBiasZ += accelZ;
      analogBiasX += (analogAccelX - analog0Level - (int)(accelX/A2D));
      delay(1000);}
  ...
  max_ang = (int)parseNextVariable(true);
  ...
  if (max_ang > 90){max_ang = 90;}//maximum 90 degree off vertical
 ...
 while (SD.exists(dataString)) {
  ...
  outputFile.print(F(","));
  outputFile.println(F("gTm,aX,aY,aZ,gX,gY,gZ,rotnX,rotnY,rotnZ,vel,alt,events,fire,pin,analog,Alt,press,temp,magX,magY,magZ,lat,lon,speed,gps_alt,gps_angle,satellites,packet"));
  if(calibrationMode){
  ...
  //sample the sensors 20 times to determine the offsets and initial values
  for (byte i = 1; i < 21; i++) { 
    //get a gyro event
    getGyro();
    getAccel();
    ...
    //sample over a period of 2 seconds
    delay(100);}

  //Divide by 20 to set the average of 20 samples
  gyroBiasX /= 20;
  gyroBiasY /= 20;
  gyroBiasZ /= 20;
  (int)(analogAccelX0 /= 20);
  accelX0 /= 20;
  accelY0 /= 20;
  accelZ0 /= 20;
  magX0 /= 20;
  ...
  if (accelZ0 - accelBiasZ >= 0) {ang_y0 = asin(min(1, (accelZ0 - accelBiasZ) / g)) * degRad;}
  else {ang_y0 = asin(max(-1, (accelZ0 - accelBiasZ) / g)) * degRad;}

  if (accelY0 - accelBiasY >= 0) {ang_z0 = asin(min(1, (accelY0 - accelBiasY) / g)) * degRad;}
  else {ang_z0 = asin(max(-1, (accelY0 - accelBiasY) / g)) * degRad;}
...
void loop(void){
  ...
  if (liftoff) {
    ...
    if(!apogee){
      ...
      ang_z += cosX * gyroZ * gdt;
      ang_z += sinX * gyroY * gdt;
      ang_y += cosX * gyroY * gdt;
      ang_y -= sinX * gyroZ * gdt;
      ...
      if (!sustainerFireCheck && rotation_OK){
        if(calcOffVert && abs(angY) > 10 && abs(angZ) > 10){
          offVert = sqrt(sq(angY) + sq(angZ));
          calcOffVert = false;}
        if (abs(angY) > max_ang || abs(angZ) > max_ang || offVert > max_ang) {rotation_OK = false;}}
    }//end if !apogee
    ...
    if (!apogee && boosterBurnout && !boosterBurnoutCheck && !pyroFire && (accel_vel < 0 || (baroApogee < 0 && accel_vel < 50))) {
      apogee = true;
      timeLastEvent = timeCurrent;}
    ...
    if (timeOut || touchdown) {
      ...
      writeFloatData(ang_y0, num6, decPts);
      writeFloatData(ang_z0, num6, decPts);
